...
def run_training(
    ...
    checkpoint_cb_stage1 = ModelCheckpoint(
        dirpath=output_dir,
        filename=cfg.MODEL.NAME + "_au_stage1_{epoch:02d}_{val/avg_f1:.4f}",
        monitor="val/avg_f1",
        mode="max",
        save_top_k=2,
    ...
    checkpoint_cb_stage2 = ModelCheckpoint(
        dirpath=output_dir,
        filename=cfg.MODEL.NAME + "_au_stage2_{epoch:02d}_{val/avg_f1:.4f}",
        monitor="val/avg_f1",
        mode="max",
        save_top_k=3,
        every_n_epochs=cfg.SOLVER.EVAL_PERIOD,
        save_last=True, # Stage 2 lưu last.ckpt để resume
    )
    ...
    if start_stage <= 2:
        ...
        # --- Export Best Model to .pth ---
        best_ckpt = checkpoint_cb_stage2.best_model_path
        if best_ckpt:
            logger.info(f"Exporting best model from {best_ckpt} to best_model.pth...")
            best_model_data = torch.load(best_ckpt, map_location="cpu")
            state_dict = best_model_data["state_dict"]
            
            # Lưu state_dict (đã được Lightning bọc model.)
            # Tuy nhiên để inference_au.py load dễ hơn, ta có thể lưu bản sạch (không có model.)
            clean_sd = {k[6:]: v for k, v in state_dict.items() if k.startswith("model.")}
            save_path = os.path.join(output_dir, "best_model.pth")
            torch.save(clean_sd, save_path)
            logger.info(f"Best model exported to {save_path}")
