...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None):
        ...
        if self.proj is not None:
            # Compute projection in fp32 to avoid fp16 overflow in AU training.
            xproj = (x12.float() @ self.proj.float()).to(x12.dtype)
            if not torch.isfinite(xproj).all():
                raise ValueError("Inf/NaN detected in VisionTransformer projection output.")
...
def build_model(state_dict: dict, h_resolution: int = 14, w_resolution: int = 14, vision_stride_size: int = 16):
